...
int removerNo(No* raiz, int chave) {
    if (raiz == NULL) return 0;

    // Verifica se a raiz é o nó a ser removido
    if (raiz->chave == chave) {
        free(raiz);
        return 1;
    }

    No* filhoAnterior = NULL;
    No* filho = raiz->primFilho;
    ...
    while (filho != NULL) {
        if (filho->chave == chave) {
            ...
            if (filhoAnterior == NULL) {
                raiz->primFilho = filho->proxIrmao;
            } else {
                filhoAnterior->proxIrmao = filho->proxIrmao;
            }
            free(filho);
            return 1;
        }

        // Procura recursivamente em cada subárvore filho
        if (removerNo(filho, chave)) {
            return 1;
        }
